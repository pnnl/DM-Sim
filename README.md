...
A Density Matrix Quantum Simulator for Single-GPU, Single-Node-Multi-GPUs and Multi-Nodes GPU Cluster. 
...
In this new version, a python-API based circuit can be directly launched and execcute without the need to recompile (which is the practice in V1.0). This will signficantly benefit emerging quantum-classical hybrid algorithms where the quantum circuits are adjusted and executed online guided by classical optimization process. In the future version we will try to provide Microsoft Q#/QDK support.

## About DM-Sim

Please see our SuperComputing (SC-20) [paper](doc/paper_sc20.pdf) for details.
...
DM-Sim (OpenMP) simulates 1M general gates with 15-qubits gate-by-gate in 94 minutes on DGX-2 (16 NVIDIA V100 GPUs) using the density-operator -- on average 5.6 ms/gate. DM-Sim simulates a VQE-UCCSD 8-qubits circuitwith 10808 gates in 249.3ms on a single NVIDIA V100 GPU -- on average 0.023 ms/gate.
...
#### **src**: DM_Sim source file
 ...
 - py_omp_wrapper.cu: Pybind11 wrapper for OpenMP/single-GPU python APIs.
 - py_mpi_wrapper.cu: Pybind11 wrapper for MPI python APIs supported by mpi4py.
 - config.hpp: configurations, constants and macros.
 - Makefile: for compilation. 
 - adder_n10_omp.cu: A 10-qubit adder example using C++/CUDA APIs (validated on DGX-2 with 4 GPUs).
 - adder_n10_omp.py: A 10-qubit adder example using Python APIs (validated on DGX-2 with 4 GPUs).
 - adder_n10_mpi.cu: A 10-qubit adder example using MPI C++/CUDA APIs (validated using 16 GPUs on Summit HPC).
 - adder_n10_mpi.py: A 10-qubit adder example using MPI Python APIs (validated using 16 GPUs on Summit HPC).
...
#### **tool**: Supporting tools (will add support for other quantum languages).
 ...
 - randomtest_n14.py: Test the single-GPU version using 100K Hadmard gates applying randomly on 14 qubits.
 - vqe_uccsd_n8.qasm: An OpenQASM based VQE circuit example genereated from [Scaffold](https://github.com/epiqc/ScaffCC). 
...
Then use the following command for compilation: 
```text
make 
```
## Execution
...
Writting a CUDA circuit code using DM-Sim C++/CUDA APIs can be simple:
```text
#include "util.cuh"
...
int main()
{
    ...
    sim.append(Simulation::H(1)); //add a Hadmard gate
...
Writting a python circuit code using DM-Sim C++/CUDA APIs can be even more simple:
...
If you find DM-Sim useful, please cite our paper:

...
DM_Sim simulates 1M general gates with 15-qubits gate-by-gate in 94 minutes on DGX-2 (16 NVIDIA V100 GPUs) using the density-opeartor -- on average 5.6 ms/gate.
...
## Configuration
You need to first config the "configuration.h" file to update *N_QUBITS* to the number of qubits in the circuit, and *GPU_SCALE* to the log2 of the number of GPUs (e.g., GPU_SCALE=2 for 4 GPUs, 3 for 8 GPUs, etc.). Note that configuration.h can be generated by our tool-script based on *configuration_sample.cuh* although configuration.h is actually used inside.
...
DM-Sim requires NVIDIA GPUs for execution. We have tested it on Tesla-P100 (Pascal, CC-6.0), Tesla-V100 (Volta, CC-7.0) and RTX2080 (Turing, CC-7.5). To run on scale-up workstation (e.g., DGX-1 and DGX-2), it needs all the GPUs to be directly connected by NVLink, NVSwitch or NV-SLI for all-to-all communication. Therefore, on DGX-1, it can use up to 4 GPUs (despite 8 in total) and provided they are directly inter-connected, see our [Evaluation paper](https://arxiv.org/abs/1903.04611) on GPU interconnect for detail. For scale-out GPU cluster, it requires the support of [GPUDirect-RDMA](https://docs.nvidia.com/cuda/gpudirect-rdma/index.html). On the ORNL Summit supercomputer, this can be enabled by *--smpiargs="-gpu"*. See the example [.lsf](src/summit_dmsim.lsf) file.
...
When build and execute correctly, the default circuit file "circuit.cuh", which realizes a rippler-carry adder using 10-qubits in total on a single-GPU, should print out the following output:
```text
...
Test-9: 1000000010
```
Here, the inputs are: carry-in cin = 0, A=0001, B=1111. The outputs are: B=B+A=0000, carry-out=1.

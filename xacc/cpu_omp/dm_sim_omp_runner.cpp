...
#undef PRINT_MEA_PER_CIRCUIT
#include "src/util_cpu.h"
#include "src/dmsim_cpu_omp.hpp"
#include <memory>

namespace DmSim {
class CpuOmp : public DmSimBackend {
public:
  virtual void init(int n_qubits, int n_threads) override {
    m_sim = std::make_shared<::DMSim::Simulation>(n_qubits, n_threads);
  }
...
std::shared_ptr<DmSimBackend> getCpuDmSim() {
  return std::make_shared<CpuOmp>();
}
} // namespace DmSim
